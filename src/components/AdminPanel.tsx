...
import { LogOut, Plus, Trash2, Edit2, Video, Image, Save, X, Calendar, User, Phone, Briefcase, LayoutGrid, List, Search, ChevronRight, BarChart3, TrendingUp, Users, Film, AlertCircle, CheckCircle2, MessageSquare, HelpCircle, Settings, Globe, Eye, Camera, MonitorSmartphone, Home } from 'lucide-react';
...
export default function AdminPanel() {
  const [activeTab, setActiveTab] = useState<'dashboard' | 'projects' | 'bookings' | 'content' | 'services' | 'testimonials' | 'faq' | 'process' | 'equipment' | 'sessions'>('dashboard');
  ...
  const [deleteType, setDeleteType] = useState<'projects' | 'services' | 'testimonials' | 'faq' | 'process' | 'equipment' | 'sessions' | null>(null);
  ...
  useEffect(() => {
    ...
    // Sessions
    const qSessions = query(collection(db, "sessions"), orderBy("createdAt", "desc"));
    const unsubscribeSessions = onSnapshot(qSessions, (snapshot) => {
      setSessions(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Session)));
    });

    // Current Session Revocation Check
    const localSessionId = localStorage.getItem('sessionId');
    let unsubscribeCurrentSession: () => void;
    if (localSessionId) {
      unsubscribeCurrentSession = onSnapshot(doc(db, "sessions", localSessionId), (docSnap) => {
        if (!docSnap.exists()) {
          // Session was revoked
          signOut(auth);
    ...
    return () => {
      ...
      unsubscribeStats();
      unsubscribeSessions();
      if (unsubscribeCurrentSession) unsubscribeCurrentSession();
    };
  }, [navigate]);
  ...
  const handleForceLogout = async () => {
    const localSessionId = localStorage.getItem('sessionId');
    if (localSessionId) {
      await deleteDoc(doc(db, "sessions", localSessionId));
    }
    signOut(auth);
    localStorage.removeItem('sessionId');
    navigate('/login');
        ...
        <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-1 bg-zinc-900/50 p-1.5 rounded-full border border-zinc-800 overflow-x-auto max-w-[200px] md:max-w-none scrollbar-hide">
            {[
              ...
              { id: 'sessions', label: 'Seanslar', icon: MonitorSmartphone }
            ].map(tab => (
              <button 
                key={tab.id}
          ...
          <button onClick={handleLogout} className="p-3 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-full transition-colors" title="Saytga qaytish">
            <Home size={20} />
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        <AnimatePresence mode='wait'>
          {activeTab === 'dashboard' ? (
            ...
            >
              ...
              <form onSubmit={handleUpdateContent} className="space-y-8">
                ...
                <div className="bg-zinc-900 p-8 rounded-3xl border border-zinc-800 space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold uppercase tracking-wider text-zinc-500">Haqida Rasmi</label>
                    <div 
                      className={`relative border-2 border-dashed rounded-xl p-8 text-center transition-all ${dragActive ? 'border-white bg-zinc-800' : 'border-zinc-800 bg-black hover:border-zinc-600'}`}
                      onDragEnter={handleDrag}
                      onDragLeave={handleDrag}
                      ...
                      onDrop={(e) => {
                        ...
                        e.stopPropagation();
                        setDragActive(false);
                        if (e.dataTransfer.files && e.dataTransfer.files[0]) handleAboutImageUpload(e.dataTransfer.files[0]);
                      }}
                    >
                      <input type="file" className="hidden" id="about-upload" accept="image/*" onChange={(e) => e.target.files && handleAboutImageUpload(e.target.files[0])} disabled={uploadingImage} />
                      
                      {content.aboutImage ? (
                        <div className="relative w-full aspect-square max-w-xs mx-auto rounded-lg overflow-hidden group">
                           <img src={content.aboutImage} alt="About" className="w-full h-full object-cover" />
                           <div className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                              <label htmlFor="about-upload" className="cursor-pointer px-4 py-2 bg-white text-black rounded-lg font-bold flex items-center gap-2">
                                 <Edit2 size={16} /> O'zgartirish
                              </label>
                           </div>
                        </div>
                      ) : (
                        <label htmlFor="about-upload" className="cursor-pointer flex flex-col items-center justify-center gap-4">
                          {uploadingImage ? <div className="w-8 h-8 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Image size={32} className="text-zinc-400" />}
                          <span className="text-zinc-500">Rasm yuklash</span>
                        </label>
                      )}
            ...
            >
              <div className="flex justify-between items-center">
                <h2 className="text-3xl font-bold">Faol Seanslar</h2>
                <button onClick={handleForceLogout} className="px-6 py-3 bg-red-500/10 text-red-500 rounded-xl font-bold hover:bg-red-500 hover:text-white transition-colors flex items-center gap-2">
                  <LogOut size={18} /> Barcha seanslarni yopish
                </button>
              </div>
              
              <div className="grid gap-4">
                {sessions.map((session) => {
                  const isCurrent = session.id === localStorage.getItem('sessionId');
                  return (
                    <div key={session.id} className={`bg-zinc-900 p-6 rounded-3xl border ${isCurrent ? 'border-white' : 'border-zinc-800'} flex items-center justify-between`}>
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center ${isCurrent ? 'bg-white text-black' : 'bg-zinc-800 text-zinc-400'}`}>
                          {session.device.includes('iPhone') || session.device.includes('Android') ? <MonitorSmartphone size={24} /> : <Home size={24} />}
                        </div>
                        <div>
                          <h3 className="font-bold text-lg flex items-center gap-2">
                            {session.device}
                            {isCurrent && <span className="text-xs bg-white text-black px-2 py-1 rounded-full font-bold uppercase tracking-wider">Joriy</span>}
                          </h3>
                          <p className="text-zinc-500 text-sm">{session.ip} • {new Date(session.createdAt).toLocaleString('uz-UZ')}</p>
                          <p className="text-zinc-600 text-xs mt-1 max-w-md truncate">{session.userAgent}</p>
                        </div>
                      </div>
                      {!isCurrent && (
                        <button 
                          onClick={() => { setDeleteId(session.id); setDeleteType('sessions'); }}
                          className="p-3 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-colors"
                          title="Seansni yopish"
                        >
                          <Trash2 size={20} />
                        </button>
                      )}
                ...
                {sessions.length === 0 && (
                  <div className="text-center py-12 bg-zinc-900 rounded-3xl border border-zinc-800">
                    <MonitorSmartphone size={48} className="mx-auto text-zinc-600 mb-4" />
                    <h3 className="text-xl font-bold mb-2">Seanslar topilmadi</h3>
                    <p className="text-zinc-500">Hozircha faol seanslar yo'q.</p>
                  </div>
                )}
              </div>
            </motion.div>
