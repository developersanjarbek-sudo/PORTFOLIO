...
import { ArrowDown, Instagram, Send, Phone, Play, ExternalLink, ArrowUpRight, Menu, X, Check, ChevronDown, Camera, Video, Monitor, Moon, Sun, Zap, Aperture, Globe, Share2, Download, Lock } from 'lucide-react';
  ...
  return (
    <div ref={containerRef} className={`${darkMode ? 'bg-black text-white selection:bg-white selection:text-black' : 'bg-white text-zinc-900 selection:bg-black selection:text-white'} min-h-screen font-sans transition-colors duration-300 overflow-x-hidden`}>
      ...
      <div className={`max-w-7xl mx-auto border-x min-h-screen relative shadow-[0_0_50px_rgba(0,0,0,0.03)] transition-colors duration-300 ${darkMode ? 'bg-black border-zinc-800' : 'bg-white border-zinc-100'}`}>
        ...
        {/* 5. Process Section */}
        <Section id="process" className={`px-6 py-24 ${darkMode ? 'bg-black' : 'bg-white'}`}>
          <FadeIn>
            <h2 className={`text-xs font-bold uppercase tracking-[0.2em] mb-16 flex items-center gap-2 ${darkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>
              <span className={`w-8 h-[1px] ${darkMode ? 'bg-zinc-700' : 'bg-zinc-300'}`}></span>
            ...
            <div className="grid md:grid-cols-4 gap-8">
              {process.length > 0 ? process.map((p, i) => (
                <div key={p.id} className="relative">
                  <div className={`text-6xl font-black mb-4 opacity-20 ${darkMode ? 'text-white' : 'text-black'}`}>{p.step}</div>
                  <h3 className={`text-xl font-bold mb-4 ${darkMode ? 'text-white' : 'text-black'}`}>{p.title}</h3>
                  <p className={`text-sm leading-relaxed ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>{p.desc}</p>
                  {i !== process.length - 1 && (
                    <div className={`hidden md:block absolute top-12 -right-4 w-8 h-[1px] ${darkMode ? 'bg-zinc-800' : 'bg-zinc-200'}`}></div>
                  )}
                </div>
              )) : (
                [1, 2, 3, 4].map(i => (
                  <div key={i} className="relative">
                    <div className={`text-6xl font-black mb-4 opacity-20 ${darkMode ? 'text-white' : 'text-black'}`}>0{i}</div>
                    <h3 className={`text-xl font-bold mb-4 ${darkMode ? 'text-white' : 'text-black'}`}>Step {i}</h3>
                    <p className={`text-sm leading-relaxed ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>Process description goes here.</p>
                  </div>
                ))
              )}
            </div>
        ...
        {/* 6. Equipment Section */}
        <Section id="equipment" className={darkMode ? 'bg-zinc-900/30' : 'bg-zinc-50/50'}>
          <FadeIn>
            <h2 className={`text-xs font-bold uppercase tracking-[0.2em] mb-16 flex items-center gap-2 ${darkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>
              <span className={`w-8 h-[1px] ${darkMode ? 'bg-zinc-700' : 'bg-zinc-300'}`}></span>
              {content.sectionTitles?.equipment || "Texnika"}
            </h2>
            
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {equipment.length > 0 ? equipment.map((eq, i) => (
                <div key={eq.id} className={`p-8 rounded-3xl border ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}>
                  <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-6 ${darkMode ? 'bg-zinc-800 text-white' : 'bg-zinc-100 text-black'}`}>
                    {getIcon(eq.icon)}
                  </div>
                  <h3 className={`text-xl font-bold mb-4 ${darkMode ? 'text-white' : 'text-black'}`}>{eq.title}</h3>
                  <ul className="space-y-2">
                    {eq.items.map((item, j) => (
                      <li key={j} className={`text-sm flex items-center gap-2 ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${darkMode ? 'bg-zinc-700' : 'bg-zinc-300'}`}></div>
                        {item}
                      </li>
                    ))}
                  </ul>
                ...
                [1, 2, 3].map(i => (
                  <div key={i} className={`p-8 rounded-3xl border ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}>
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center mb-6 ${darkMode ? 'bg-zinc-800 text-white' : 'bg-zinc-100 text-black'}`}>
                      <Camera size={24} />
                    </div>
                    <h3 className={`text-xl font-bold mb-4 ${darkMode ? 'text-white' : 'text-black'}`}>Category {i}</h3>
                    <ul className="space-y-2">
                      <li className={`text-sm flex items-center gap-2 ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${darkMode ? 'bg-zinc-700' : 'bg-zinc-300'}`}></div>
                        Item 1
                      </li>
                    </ul>
        ...
        <Section id="testimonials" className={`px-6 py-24 ${darkMode ? 'bg-black' : 'bg-white'}`}>
          <FadeIn>
            <h2 className={`text-xs font-bold uppercase tracking-[0.2em] mb-16 flex items-center gap-2 ${darkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>
              <span className={`w-8 h-[1px] ${darkMode ? 'bg-zinc-700' : 'bg-zinc-300'}`}></span>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              {testimonials.length > 0 ? testimonials.map((t, i) => (
                <div key={t.id} className={`p-8 rounded-3xl border ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}>
                  <p className={`text-lg italic mb-8 ${darkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>"{t.text}"</p>
                  <div className="flex items-center gap-4">
                    <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold ${darkMode ? 'bg-zinc-800 text-white' : 'bg-zinc-200 text-black'}`}>
                      {t.name.charAt(0)}
                    </div>
                    <div>
                      <h4 className={`font-bold ${darkMode ? 'text-white' : 'text-black'}`}>{t.name}</h4>
                      <p className={`text-sm ${darkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>{t.role}</p>
                    </div>
                  </div>
                </div>
              )) : (
                [1, 2].map(i => (
                  <div key={i} className={`p-8 rounded-3xl border ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}>
                    <p className={`text-lg italic mb-8 ${darkMode ? 'text-zinc-300' : 'text-zinc-700'}`}>"Great work!"</p>
                    <div className="flex items-center gap-4">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold ${darkMode ? 'bg-zinc-800 text-white' : 'bg-zinc-200 text-black'}`}>
                        C
                      </div>
                      <div>
                        <h4 className={`font-bold ${darkMode ? 'text-white' : 'text-black'}`}>Client Name</h4>
                        <p className={`text-sm ${darkMode ? 'text-zinc-500' : 'text-zinc-500'}`}>Role</p>
                      </div>
                    </div>
                  </div>
                ))
        ...
        {/* 8. FAQ Section */}
        <Section id="faq" className={darkMode ? 'bg-zinc-900/30' : 'bg-zinc-50/50'}>
          <FadeIn>
            <h2 className={`text-xs font-bold uppercase tracking-[0.2em] mb-16 flex items-center gap-2 ${darkMode ? 'text-zinc-500' : 'text-zinc-400'}`}>
              <span className={`w-8 h-[1px] ${darkMode ? 'bg-zinc-700' : 'bg-zinc-300'}`}></span>
              {content.sectionTitles?.faq || "FAQ"}
            </h2>
            
            <div className="max-w-3xl mx-auto space-y-4">
              {faq.length > 0 ? faq.map((f, i) => (
                <details key={f.id} className={`group p-6 rounded-2xl border ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}>
                  <summary className={`flex justify-between items-center font-bold cursor-pointer list-none ${darkMode ? 'text-white' : 'text-black'}`}>
                    {f.q}
                    <span className="transition group-open:rotate-180">
                      <ChevronDown size={20} />
                    </span>
                  </summary>
                  <p className={`mt-4 text-sm leading-relaxed ${darkMode ? 'text-zinc-400' : 'text-zinc-500'}`}>
                    {f.a}
                  </p>
                </details>
              )) : (
                [1, 2, 3].map(i => (
                  <details key={i} className={`group p-6 rounded-2xl border ${darkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-100'}`}>
                    <summary className={`flex justify-between items-center font-bold cursor-pointer list-none ${darkMode ? 'text-white' : 'text-black'}`}>
                      Question {i}?
                      <span className="transition group-open:rotate-180">
                        <ChevronDown size={20} />
